...
namespace TextHub
{
    ...
    /// </summary>
    class TextHubProjectXML : TextHubProject
    {
        ...
        public TextHubProjectXML(string name, string folderPath, string currentVersionPath)
        {
            Title = name;
        ...
        /// <param name="name">New project name</param>
        /// <param name="folderPath">The full path to the main project folder</param>
        public TextHubProjectXML(string name, string folderPath)
        {
            Title = name;
            Versions = new ObservableCollection<TextHubVersion>();
            projectFolderPath = folderPath;
        ...
        {
            string title = TextHubProject.MakeNewProject(newFolderPath, ".xml");
            return new TextHubProjectXML(title, newFolderPath, title + ".xml");
        }

        /// <summary>
        /// Parses an .xml file into a new project.
        /// </summary>
        ...
        public new static TextHubProject ParseFile(string filePath)
        {
            ...
            return new TextHubProjectXML(attrs[0], attrs[1], attrs[2]);
        }
        ...
        public new static TextHubProject ParseProject(string folderPath)
        {
            ...
            if (!noPointer)
            {
                TextHubProject project = new TextHubProjectXML(projectName, folderPath);
                try
                {
                    project.ParseVersionsFromFile(pointerFullPath, ".xml");
                }
                catch (Exception ex)
                {
            ...
            {
                TextHubProject.NoPointerToProject(folderPath, ".xml");
                TextHubProject project = new TextHubProjectXML(projectName, folderPath);
                project.ParseVersionsFromFile(pointerFullPath, ".xml");
                return project;
            }
        ...
        {
            SaveNewVersion(newVersionName, ".xml");
        }
        ...
        public override TextHubProject MakeSubproject(TextHubVersion textHubVersion, string newName)
        {
            ...
            TextHubProject project = new TextHubProjectXML(newName, newPath);
            ...
            for (int i = 0; i < Versions.Count && Versions[i] != textHubVersion; ++i)
            {
                if (File.Exists(projectFolderPath + Path.DirectorySeparatorChar + Versions[i].RelativeFilePath))
                {
                    File.Copy(projectFolderPath + Path.DirectorySeparatorChar + Versions[i].RelativeFilePath,
                        newPath + Path.DirectorySeparatorChar + versionsDirectoryName + Path.DirectorySeparatorChar + Versions[i].Title + ".xml");
                    versionPaths.Add(versionsDirectoryName + Path.DirectorySeparatorChar + Versions[i].Title + ".xml");
                    project.Versions.Add(new TextHubVersion(versionsDirectoryName + Path.DirectorySeparatorChar + Versions[i].Title + ".xml", counter++, false, project));
                }
            }

            File.Copy(projectFolderPath + Path.DirectorySeparatorChar + textHubVersion.RelativeFilePath,
                newPath + Path.DirectorySeparatorChar + newName + ".xml");
            versionPaths.Add(newName + ".xml");
            project.Versions.Add(new TextHubVersion(newName + ".xml", counter, true, project));
